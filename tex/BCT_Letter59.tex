%% BCT Letter 59 — Ultralight Dark Matter
...
\date{March 2026 \quad BCT Letter 59}
...
\[
  ...
  = \frac{(220.44\ \mathrm{MeV})^2}{1.221\times10^{22}\ \mathrm{MeV}}
  = 3.97\times10^{-12}\ \mathrm{eV},
\]
corresponding to a coherent electromagnetic oscillation at $f_{\rm BCT} = m_{\rm DM}c^2/h = 962\ \mathrm{Hz}$.
...
This is a \emph{second} BCT dark matter candidate, distinct from the D4 domain-wall
candidate at 62.9~GeV (Letter~30~\cite{L30}): the phonon zero mode is ultralight
and detectable at audio frequencies rather than at collider or direct-detection scales.
This is Prediction~\#117 of the BCT programme. \textit{Zero free parameters.}
\end{abstract}
...
The BCT Superfluid Lattice Model~\cite{Monograph,PRL} predicts two distinct
dark matter candidates from its geometry: (1) D4 domain walls at $62.9\ \mathrm{GeV}$
detectable at LZ and XENONnT (Letter~30~\cite{L30}, Prediction~\#30); and (2) the
ultralight acoustic phonon zero mode described here. Both arise from the same BCT
condensate geometry; they occupy completely different mass scales and require
completely different detection strategies.
The zero-mode acoustic phonon mass is set by the geometric mean of the two BCT
energy scales already determined by the programme.
...
\begin{equation}
  ...
  \label{eq:mDM}
\end{equation}
Substituting BCT values $\Lambda_{\rm QCD} = 220.44\ \mathrm{MeV}$ and
$m_P({\rm BCT}) = 1.221\times10^{22}\ \mathrm{MeV}$ (Letter~35~\cite{L35}):
\begin{equation}
  m_{\rm DM} = \frac{(220.44)^2}{1.221\times10^{22}}\ \mathrm{MeV}
  = 3.979\times10^{-12}\ \mathrm{eV}
  \label{eq:mDMnum}
\end{equation}
...
\begin{equation}
  ...
  = \frac{3.979\times10^{-12}\ \mathrm{eV}}{4.136\times10^{-15}\ \mathrm{eV\cdot s}}
  = 962\ \mathrm{Hz}
  \label{eq:fBCT}
\end{equation}
...
\begin{equation}
  ...
  = \frac{1.221\times10^{22}\ \mathrm{MeV}}{\sqrt{51.003}}
  = 1.709\times10^{18}\ \mathrm{GeV}
  \label{eq:fa}
\end{equation}
The photon coupling is:
\begin{equation}
  g_{\phi\gamma\gamma} = \frac{\alpha_{\rm EM}}{2\pi f_a}
  = \frac{1/137.036}{2\pi \times 1.709\times10^{18}\ \mathrm{GeV}}
  = 6.81\times10^{-31}\ \mathrm{GeV}^{-1}
  \label{eq:gcoupling}
\end{equation}
...
\begin{table}[h]
\centering
\caption{BCT ultralight dark matter vs.\ running experiments.}
\label{tab:expt}
\begin{tabular}{lccc}
\toprule
Experiment & Range (eV) & BCT $m_{\rm DM}$ & Status \\
\midrule
ABRACADABRA & $10^{-12}$--$10^{-9}$ & $3.98\times10^{-12}$ & \textbf{Target} \\
DM Radio     & $10^{-12}$--$10^{-6}$ & $3.98\times10^{-12}$ & \textbf{Target} \\
...
A signal at $962 \pm 1\ \mathrm{Hz}$ in a toroidal SQUID detector would confirm
the BCT ultralight dark matter candidate. Non-detection with sensitivity below
...
an open calculation; the mass prediction~(\ref{eq:mDM}) is
independent of the production mechanism.

\medskip
\noindent\textbf{Relation to D4 Domain-Wall Dark Matter.}
BCT predicts two dark matter candidates at vastly different scales:
\begin{itemize}
  \item \textbf{Heavy:} D4 domain walls at $62.9\ \mathrm{GeV}$~\cite{L30}.
    Detection: LZ, XENONnT (nuclear recoil). Prediction~\#30.
  \item \textbf{Ultralight:} acoustic phonon zero mode at $3.98\times10^{-12}\ \mathrm{eV}$.
    Detection: ABRACADABRA, DM Radio (SQUID magnetometry). Prediction~\#117.
\end{itemize}
Both arise from the BCT condensate geometry with zero free parameters.
If ABRACADABRA detects a 962~Hz signal \emph{and} LZ detects a 62.9~GeV
WIMP, BCT predicts both simultaneously --- a smoking gun for the framework.

\medskip
\noindent\textbf{Falsifiable Predictions.}
\begin{enumerate}
\item Signal at $f = 962 \pm 10\ \mathrm{Hz}$ in ABRACADABRA or DM Radio.
\item No signal at ADMX ($10^{-6}$--$10^{-5}\ \mathrm{eV}$ band):
  BCT mass is $10^6\times$ too light.
\item No signal at CASPEr-E ($10^{-17}$--$10^{-14}\ \mathrm{eV}$ band):
  BCT mass is $10^2\times$ too heavy.
\item If ABRACADABRA detects a signal outside $[100,\,9600]\ \mathrm{Hz}$,
  the BCT ultralight dark matter candidate is falsified.
\end{enumerate}

\medskip
\noindent\textbf{Conclusion.}
The BCT superfluid phonon zero mode has mass
$m_{\rm DM} = \Lambda_{\rm QCD}^2/m_P = 3.98\times10^{-12}\ \mathrm{eV}$,
corresponding to a coherent oscillation at $962\ \mathrm{Hz}$.
Its photon coupling $g_{\phi\gamma\gamma} = 6.81\times10^{-31}\ \mathrm{GeV}^{-1}$
...
This is a second BCT dark matter candidate, complementary to the D4 domain-wall
candidate at 62.9~GeV. All parameters follow from $r_{\rm oct}$, $r_{\rm tet}$,
and $\Lambda_{\rm QCD}$. Prediction~\#117. Zero free parameters.

\begin{thebibliography}{99}
\bibitem{PRL} M.~R.~Cabri\'e, BCT PRL Letter (2026), DOI:~10.5281/zenodo.18884415.
\bibitem{Monograph} M.~R.~Cabri\'e, BCT Monograph (2026), DOI:~10.5281/zenodo.18884976.
\bibitem{L30} M.~R.~Cabri\'e, BCT Letter~30 (2026). D4 domain-wall dark matter at 62.9~GeV.
\bibitem{L35} M.~R.~Cabri\'e, BCT Letter~35 (2026). Newton's gravitational constant.
