...
Applying Eq.~\eqref{eq:x_formula} with $R = \roct$ and
$S_{\mathrm{bare}} = S_{EW} = \pi^5/12$:
\begin{equation}
  x_{EW}^{(0)} = \frac{4\pi^3\roct}{3S_{EW}} = 0.33575.
\end{equation}
The NLO correction follows the universal BCT pattern
$\mathcal{O} \to \mathcal{O}(1 + N\azero)$~\cite{appFE},
with $N = (4\pi+1)/\pi^2$ (the D4--U(1) loop factor):
\begin{equation}
  \boxed{x_{EW} = \frac{4\pi^3\roct}{3S_{EW}}\left(1+\NLO\right) = 0.33917,}
  \label{eq:xEW}
\end{equation}
where $\NLO \equiv \azero(4\pi+1)/\pi^2 = 0.010183$.
The App.~AY back-solved value is $x_{EW}^{\mathrm{AY}} = 0.33934$;
Eq.~\eqref{eq:xEW} agrees to $\mathbf{0.052\%}$.

\subsection*{Physical interpretation}
The factor $4\pi^3/3 = V_{\mathrm{sphere}}/R^3 \times \pi^2/R^2 \times R^3$
is the volume of a unit sphere times the Laplacian eigenvalue, divided by the 
void volume: $x_{EW}$ measures how efficiently the EW condensate fills 
the octahedral void relative to the instanton action.
The NLO carries opposite sign to $x_{SU(3)}$ (see below), reflecting 
the opposite chirality of SU(2)$_L$ vs.\ SU(3)$_c$.
...
\begin{equation}
  v_0 = \mP\exp\!\left(\frac{-S_{EW}}{1-x_{EW}}\right)
  = 212.4\;\text{GeV}.
  \label{eq:v0}
\end{equation}
This is the EW condensate VEV at the BCT lattice scale.
The physical value $v = 246.2$~GeV is obtained after
electroweak radiative corrections from the condensate scale
to the Fermi scale (App.~FU/Phase~37C~\cite{appFU}):
\begin{equation}
  v^2 = v_0^2\left(1 + \beta_v\ln\frac{\mP}{v}\right),
  \label{eq:v_running}
\end{equation}
where $\beta_v = (6y_t^2 - \frac{9}{4}g_W^2 - \frac{3}{4}g'^2 -
3\lambda_H)/(8\pi^2)$ with all couplings BCT-derived.
This is BCT Prediction~\#169. The full derivation is in App.~FU.

% ─────────────────────────────────────────────────────────────────────
\section{The Symmetry of the NLO Signs}
% ─────────────────────────────────────────────────────────────────────

A structural observation: the NLO corrections have \emph{opposite
signs} for the EW and QCD sectors:
\begin{align}
  x_{EW}   &\propto (1 + \NLO) \quad \text{[SU(2), octahedral, left-chiral]},\\
  x_{SU(3)}&\propto (1 - \NLO) \quad \text{[SU(3), tetrahedral, vector-like]}.
\end{align}
This sign flip reflects a deep structural property of the BCT vacuum:
the SU(2)$_L$ gauge symmetry is \emph{chiral} (couples to left-handed
fermions only), while SU(3)$_c$ is \emph{vector-like} (couples to
both chiralities). In BCT, chirality is encoded by the void geometry:
the octahedral void has a preferred orientation (the $z$-axis of the
$D_{4h}$ tetragonal lattice), while the tetrahedral void is
orientation-symmetric. The NLO sign encodes this geometric chirality.
...
$x_{EW}$ & Eq.~\eqref{eq:xEW} & $0.33917$ & $0.052\%$ \\
$x_{SU(3)}$ & Eq.~\eqref{eq:xSU3} & $0.71954$ & $0.007\%$ \\
$v_0$ & Eq.~\eqref{eq:v0} & $212.4$ GeV & EW rad.\ corr. \\
$\LQCD$ & Eq.~\eqref{eq:LQCD} & $219.83$ MeV & $\mathbf{-0.079\%}$ \\
\midrule
$v_{\mathrm{obs}}$ & --- & $246.2$ GeV & --- \\
$\LQCD^{\mathrm{obs}}$ & --- & $220.0$ MeV & --- \\
\bottomrule
\end{tabular}
\end{table}

The QCD confinement scale is derived from first principles at
$-0.079\%$. The EW condensate scale is derived at $-13.7\%$ before
electroweak radiative corrections; with corrections (App.~FU),
$v = 246.2$~GeV follows. All inputs are geometric:
$\roct$, $\rtet$, and $\mP$. Zero free parameters.
