...
[Cold Ledger correction, 16 Sep 2026 --- Gate EDM / Gate THETA.] The result
$\thetabar_{\mathrm{QCD}} = 0$ is tiered \textbf{CONJECTURE}, not proven. The three
``proofs'' below establish only that the $D_4\,\mathbb{Z}_2 / T_d$ structure fixes
$\theta$ to the CP-conserving set $\{0,\pi\}$; $\theta=\pi$ is excluded by
\emph{observation}, not by the geometry, so $\theta=0$ is not a pure geometric output.
Proof~2 (``real Yukawa couplings $\Rightarrow \arg\det M_q = 0$'') is a non-sequitur:
$\arg\det M_q$ is not fixed by the reality of the physical masses --- a nonzero
CKM phase ($J_{\mathrm{CKM}} = 3.08\times10^{-5}$) requires a complex
mass matrix, and reconciling $\arg\det M_q = 0$ with $J_{\mathrm{CKM}}\neq 0$ from one
$T_d$ texture is the Barr criterion, which is \emph{imposed}, not derived here.
The neutron-EDM consequence is a CONJECTURE-tier floor $d_n \lesssim 10^{-30}$~e$\cdot$cm
(a detection $\gtrsim 10^{-28}$ falsifies), not $d_n = 0$ exact; the ``no QCD axion''
statement inherits this tier.
\end{quote}
...
\emph{[Cold Ledger correction, 16 Sep 2026: this naive estimate is
$\sim\!13\times$ \textbf{above} the $1.8\times10^{-10}$ bound quoted above,
not below it --- the original ``already below the experimental bound'' is a
false inequality. Moreover $J/(16\pi^2)$ is not the correct SM contribution to
$\thetabar$, which is far smaller and first enters at high loop order; the naive
estimate should not be read as the physical radiative shift. See Gate THETA.]}
Higher loops are suppressed by powers of $\azero \approx 0.007$.
