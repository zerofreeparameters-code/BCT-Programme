...
in quantum mechanics. This maximum arises when measurement
directions are separated by $45°$ in the Hopf fibre bundle.

In BCT, the optimal measurement angle carries a precise
geometric signature. The CHSH optimum requires:
\begin{equation}
\cos\frac{\pi}{4} = \frac{1}{\sqrt{2}} = \frac{1}{2\roct + 1}
\label{eq:tsirelson}
\end{equation}
This is an \emph{exact algebraic identity}, following
immediately from $\roct = (\sqrt{2}-1)/2$:
\begin{align}
2\roct + 1 &= (\sqrt{2}-1) + 1 = \sqrt{2} \nonumber \\
\Rightarrow\quad \frac{1}{2\roct+1} &= \frac{1}{\sqrt{2}} = \cos\frac{\pi}{4}
\end{align}
The Tsirelson bound follows:
$2/\cos(45°) = 2\sqrt{2}$.

The optimal Bell measurement angle is geometrically encoded
in the BCT octahedral void radius. This is not a free
parameter: $\roct$ is fixed by Lorentz invariance and the
requirement of isotropic phonon propagation in the BCT
lattice~\cite{monograph}. Terminal-verified to 10 significant
figures. Tier: \textsc{Proven} (algebraic identity).

\subsection{Entanglement Entropy as Hopf Complexity}

Entanglement entropy $S = -\mathrm{Tr}(\rho \log \rho)$
measures the topological complexity of the shared Hopf
configuration. Maximum entanglement ($S = \log 2$
for a qubit pair) corresponds to a single irreducible
$H=0$ winding connecting two $H=\pm 1$ projections.
