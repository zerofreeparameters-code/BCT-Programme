...
%
% [Cold Ledger correction, 16 Sep 2026 -- Gate EDM / Gate THETA]:
% $\thetabar_{\mathrm{QCD}} = 0$ is CONJECTURE, not proven. (i)-(iii) fix $\theta$
% only to $\{0,\pi\}$; $\pi$ is excluded by observation, not geometry. (ii) is a
% non-sequitur: real physical masses do not fix $\arg\det M_q$ when $J_{\mathrm{CKM}}
% \neq 0$ (the Barr criterion, imposed not derived). The EDM claim is a
% CONJECTURE-tier floor $d_n \lesssim 10^{-30}$ (detection $\gtrsim 10^{-28}$
% falsifies), not $d_n = 0$ exact; ``no axion'' inherits this tier.
